...
The user must set configuration variables in the \tt makefile \rm build script to
allow the Hog compiler to use the java compiler on the user's computer. The Hog
compiler only requires the user to have a working installation of Java on their computer,
which generates the job jar, which can be run on the Amazon ElasticMapReduce cloud.
