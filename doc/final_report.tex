...
To build and run a Hog source file there is a makefile and a shell script
which builds and runs the Hog source file. If the user has a working Hadoop
installation on their computer, the user must edit the makefile to insert the 
name of the Hog source program and configure the paths unique to their system,
and then the user can type "make compile" , which will compile the Hog source 
program into the Hog.java file. Then the user will type "make" which has a default 
of using the javac compiler to compile the Java source program generated by the Hog compiler, with the Hadoop libraries, and then copies the input files into the distributed file system and runs the job on 
the local node and reports the output. \\

Shell Script Usage: \tt Hog.sh sourcename.hog  \rm \\
Makefile Usage: \tt make compile \rm \\
Makefile Usage: \tt make \rm \\
...
\begin{quotation}
\noindent A node representing unary operators. Written by Samuel Messing.
\end{quotation}
