...
can express the same computation in about 10 lines.
...
single string. Hog implements concatenation by use of the Java \tt StringBuilder \rm (or \tt StringBuffer\rm) class and its
...
Not all basic types can be cast to a different basic type. Any variable of type
\tt int \rm or \tt real \rm can be cast to any of the other basic types (\tt
int\rm, \tt real\rm, \tt bool\rm, \tt text\rm). Note, when casting from a \tt real
\rm to an \tt int\rm, the fractional part of the \tt real \rm value will be
\textbf{\emph{truncated}}. However, a variable of type bool can only be cast to
\tt text \rm and a variable of type \tt text \rm can be cast to an \tt int \rm or
\tt real\rm. If casting a \tt text \rm to an \tt int \rm or \tt real \rm results
in an invalid number (e.g. \tt (int) "1a4"\rm), then a \tt TypeCaseException \rm
will be thrown.
...
\begin{itemize}
  \item[] \tt IncorrectArgumentError \rm
  ...
  \item[] \tt ProgramStructureError \rm
  
  Thrown when a Hog program does not follow the structure described earlier in \S \ref{sec:program_structure}.

  \item[] \tt NoSuchVariableError \rm
  
  Thrown when an attempt is made to use a variable that has not been declared yet.

  \item[] \tt NoSuchFunctionError \rm
  
  Thrown when a function is invoked that does not exist.

  \item[] \tt UnsupportedOperationError \rm
